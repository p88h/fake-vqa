...
def fetch_image(url: str) -> Image:
    media_connector = MediaConnector(allowed_local_media_path=pathlib.Path.cwd().as_posix())
    return media_connector.fetch_image(url)
...
def load_qwen2_5_vl(question: str, image_urls: list[str]) -> ModelRequestData:
    ...
    engine_args = EngineArgs(
        ...
        max_num_seqs=8,
        limit_mm_per_prompt={"image": len(image_urls)},
        # customizations to fit in low-memory GPU
