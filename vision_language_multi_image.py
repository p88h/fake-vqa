...
from vllm import LLM, EngineArgs, AsyncEngineArgs, SamplingParams
...
def load_keye_vl(question: str, image_urls: list[str]) -> ModelRequestData:
    ...
    processor = get_processor(model_name, trust_remote_code=True)
...
def local_fetch_image(url: str) -> Image.Image:
    img = Image.open(url)
    img.load()
...
def load_qwen2_5_vl(question: str, image_urls: list[str]) -> ModelRequestData:
    ...
    if smart_resize is None:
        ...
        def post_process_image(image: Image.Image) -> Image.Image:
            width, height = image.size
            resized_height, resized_width = smart_resize(
                height, width, max_pixels=1000 * 28 * 28
            )
            return image.resize((resized_width, resized_height))

        image_data = [post_process_image(local_fetch_image(url)) for url in image_urls]
